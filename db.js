...
// Add 'variants' column to products if missing
try {
  const cols = db.prepare("PRAGMA table_info(products)").all();
  const names = cols.map(c => c.name);
  if (!names.includes('variants')) {
    db.exec("ALTER TABLE products ADD COLUMN variants TEXT DEFAULT NULL");
    console.log('[db] Migrated: added variants to products');
  }
  if (!names.includes('description')) {
    db.exec("ALTER TABLE products ADD COLUMN description TEXT DEFAULT ''");
    console.log('[db] Migrated: added description to products');
  }
  if (!names.includes('stock')) {
    db.exec("ALTER TABLE products ADD COLUMN stock INTEGER DEFAULT 100");
    console.log('[db] Migrated: added stock to products');
  }
  if (!names.includes('active')) {
    db.exec("ALTER TABLE products ADD COLUMN active INTEGER DEFAULT 1");
    console.log('[db] Migrated: added active to products');
  }
} catch (e) { console.warn('[db] products migration failed:', e.message); }

// Add 'variant' column to order_items if missing
try {
  ...
  if (!cols.some(c => c.name === 'variant')) {
    ...
    console.log('[db] Migrated: added variant to order_items');
  }
} catch (e) { console.warn('[db] order_items migration failed:', e.message); }
